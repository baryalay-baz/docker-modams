...
(function (w, $) {
    ...
    U.makeDataTable = function (tableRef, type = "1", recordsPerPage = 10, actionsConfig) {
        ...
        function buildRowActionsHtml(buttons) {
            ...
            ${buttons.map((b) => {
                // sensible default variant per key to avoid plain .btn (which turns white on hover)
                const keyLc = String(b.key || "").toLowerCase();
                const fallbackByKey = {
                    edit: " btn-outline-primary",
                    lock: " btn-outline-danger",
                    unlock: " btn-outline-success",
                    view: " btn-outline-secondary",
                };
                let extra = fallbackByKey[keyLc] || " btn-outline-light";   // safe baseline
                if (b.className) extra = " " + U.escapeHtml(b.className);   // explicit class wins
